...
cModule *MigrationMecServiceSelectionBased::findBestMecHost(const ApplicationDescriptor& appDesc)
{
    ...
    for (auto mecHost : mecOrchestrator_->mecHosts) {
        EV << "MigrationMecServiceSelectionBased::findBestMecHost - MEC host [" << mecHost->getName() << "] size of mecHost " << mecOrchestrator_->mecHosts.size() << endl;
...
cModule *MigrationMecServiceSelectionBased::findNewBestMecHost(const ApplicationDescriptor& appDesc, const char* oldMecHost)
{
    ...
    for (auto mecHost : mecOrchestrator_->mecHosts) {
        ...
        if (!strcmp(mecHost->getName(), oldMecHost))
            continue;
