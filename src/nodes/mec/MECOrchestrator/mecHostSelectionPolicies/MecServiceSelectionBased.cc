...
cModule *MecServiceSelectionBased::findBestTargetMecHost(const ApplicationDescriptor& appDesc, std::vector<cModule *> eligibleTargetMecHosts) {
    EV << "MecServiceSelectionBased::findBestTargetMecHost - finding best MecHost..." << endl;
    cModule *bestHost = nullptr;
    bool found = false;

    for (auto mecHost : eligibleTargetMecHosts) {
        EV << "MecServiceSelectionBased::findBestTargetMecHost - MEC host [" << mecHost->getName() << "] size of mecHost " << eligibleTargetMecHosts.size() << endl;
        VirtualisationInfrastructureManager *vim = check_and_cast<VirtualisationInfrastructureManager *>(mecHost->getSubmodule("vim"));
        ResourceDescriptor resources = appDesc.getVirtualResources();
        bool res = vim->isAllocable(resources.ram, resources.disk, resources.cpu);
        if (!res) {
        ...
        if (appDesc.getAppServicesRequired().size() > 0) {
            serviceName = appDesc.getAppServicesRequired()[0];
            EV << "MecServiceSelectionBased::findBestTargetMecHost - required Mec Service: " << serviceName << endl;
        }
        else {
            EV << "MecServiceSelectionBased::findBestTargetMecHost - the Mec App does not require any MEC service. Choosing the temporary Mec Host as the best one" << endl;
            found = true;
            break;
        }
        for (const auto& service : *mecServices) {
            if (serviceName == service.getName() && service.getMecHost() == bestHost->getName()) {
                EV << "MecServiceSelectionBased::findBestTargetMecHost - The temporary Mec Host has the MEC service " << service.getName() << " required by the Mec App. It has been chosen as the best one" << endl;
                bestHost = mecHost;
                found = true;
                break;
            }
    ...
    if (bestHost != nullptr && !found)
        EV << "MecServiceSelectionBased::findBestTargetMecHost - The best Mec Host does not have the required service. Best MEC host: " << bestHost << endl;
    else if (bestHost == nullptr)
        EV << "MecServiceSelectionBased::findBestTargetMecHost - no MEC host found" << endl;
