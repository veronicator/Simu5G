...
void HostMobilityService::handleCellChangeNotification(const nlohmann::ordered_json& request) {
    ...
    if (res) {
        ...
        auto eraseElem = eNodeB_.erase(binder_->getModuleByMacNodeId(srcEcgi.getCellId()));
        // add trg cell in the set
...
void HostMobilityService::handleHostMobilityUpdate() {
    EV_DEBUG << "HostMobilityService::handleHostMobilityUpdate" << endl;
